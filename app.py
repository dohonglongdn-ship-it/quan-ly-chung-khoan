...
st.title("📈 Hệ thống Phân tích Chứng khoán Pro (Time-Sync Engine)")
...
# --- VŨ KHÍ MỚI: ÉP BUỘC THỜI GIAN THỰC TẾ (CHỐNG LỖI 404 NOT FOUND) ---
@st.cache_data(ttl=900, show_spinner=False)
def lay_du_lieu(ma):
    loi_chi_tiet = []
    
    # ÉP MỐC THỜI GIAN VỀ NĂM 2023 - 2024 ĐỂ TRÁNH MÁY CHỦ BỊ LỆCH SANG 2026
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if data.get('t') and len(data['t']) > 0:
                ...
                loi_chi_tiet.append("SSI: Trả về dữ liệu rỗng (Có thể mã không giao dịch).")
        else:
            loi_chi_tiet.append(f"SSI từ chối (Lỗi {res.status_code})")
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            loi_chi_tiet.append(f"DNSE từ chối (Lỗi {res.status_code})")
    except Exception as e:
...
with tab1:
    ...
    with st.spinner("Đang đồng bộ hóa mốc thời gian thực tế..."):
