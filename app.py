...
# --- MODULE 3: LẤY ĐÁNH GIÁ TỪ CHUYÊN GIA TCBS ---
@st.cache_data(ttl=86400, show_spinner=False)
def lay_danh_gia_tcbs(ma):
    try:
        # Sử dụng API phân tích công cộng của Techcom Securities
        url = f"https://apipubaws.tcbs.com.vn/tcanalysis/v1/rating/{ma}"
        res = requests.get(url, headers={'User-Agent': 'Mozilla/5.0'}, timeout=3)
        if res.status_code == 200:
            data = res.json()
            return data.get('generalRating', 0)
    except: pass
    return 0
...
with tab2:
    if ma_chon:
        ...
        pe_hien_thi, pb_hien_thi = profile.get('pe'), profile.get('pb')
        von_hoa_ty = profile.get('marketCap', 0) / 1_000_000_000
        
        df, _, _ = lay_du_lieu_bieu_do(ma_chon)
        if not df.empty:
            gia_hien_tai, klgd_20 = df['Close'].iloc[-1], df['Volume'].tail(20).mean()
...
with tab3:
    ...
    if DANH_SACH_MA:
        ...
        # TÍNH NĂNG MỚI: BÁO CÁO CỐ VẤN CHUYÊN SÂU
        st.markdown("---")
        ...
        if ma_phan_tich != "-- Chọn mã --":
            with st.spinner(f"Đang tổng hợp dữ liệu Phân tích Kỹ thuật, Cơ bản và Đánh giá chuyên gia cho {ma_phan_tich}..."):
                ...
                if not df_pt.empty and len(df_pt) >= 50:
                    ...
                    pb_pt = ho_so_pt.get('pb', 'N/A')
                    
                    # Lấy Đánh giá chuyên gia TCBS
                    ...
                    # --- BỘ LẬP LUẬN AI BÁO CÁO ---
                    # 1. Luận điểm Kỹ thuật
...
with tab4:
    ...
    if DANH_SACH_MA:
        ...
        if st.button("💾 Lưu Cấu Hình"): luu_danh_muc_vao_o_cung(du_lieu_cap_nhat); st.rerun()
